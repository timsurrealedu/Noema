...
- The first local backend slice now exists: Node SQLite/WAL persistence, FTS5, single-owner scrypt authentication and hashed sessions, idempotent/version-checked core APIs, durable jobs, a schema-bound Codex worker, and a Bubblewrap-isolated compiler. Captures close the full loop: interpretation proposals apply transactionally via `/captures/:id/apply` or `/jobs/:id/approve`, every mutation writes a reversible audit event (`/audit`, `/audit/:id/undo`), and job progress streams over SSE (`/jobs/:id/events`). Uploads, deterministic extraction, session management, job retries, and the offline queue are done. Frontend core state hydrates from and mirrors to the API when authenticated, while retaining browser-local fallback. External integrations, production hardening, and deployment remain.
- Implemented frontend milestone: Today/Quick Capture, Capture Inbox, advanced Tasks and Calendar views, persisted Markdown Vault, Study workflows, Canvas, project/repository/automation workspaces, global search, notifications, activity/undo, contextual AI planning, Settings/security/data controls, authentication, Help, route states, and PWA shell. Core objects, captures, assets, audit, and sessions are server-backed; Study/Projects/Automations integrations remain simulated.
- Next priorities, in order: (1) projects table + relationship joins, note version history, backlink index, dedicated `/search`; (2) ops slice — health/backup/structured logs, systemd + Tailscale/Caddy deployment, v1 importer; (3) study backend, notifications, automations; (4) frontend verification (browser interaction suite, WCAG audit, pixel baselines, device PWA check).

...
export default function CodingPage(){
  ...
  return <ModuleShell active="Coding" title="Coding" action={<Link className="primary top-primary" href="/coding/compiler"><Code/>New compiler run</Link>}>
    ...
    <section><div className="list-title"><h3>Repositories</h3><span>{repositories.length} registered</span></div>
      <form className="repo-register" onSubmit={register}><label>Name <input value={name} onChange={event=>setName(event.target.value)} placeholder="LifeOS" maxLength={100}/></label><label>Allowed local path <input value={path} onChange={event=>setPath(event.target.value)} placeholder="/srv/code/lifeos" required/></label><button className="primary"><Plus/>Register</button></form>
      {error&&<p className="auth-error" role="alert">{error}</p>}{loading?<p role="status">Loading repositories…</p>:repositories.length?<div className="repo-cards">{repositories.map(repo=><Link href={`/coding/repositories/${repo.id}`} className="repo-card" key={repo.id}><FolderOpen/><span><strong>{repo.name}</strong><small>{repo.path}</small></span><ArrowRight/></Link>)}</div>:<div className="empty-state"><GitBranch/><h3>No repositories registered</h3><p>Register a path inside <code>LIFEOS_REPOSITORY_ROOTS</code>.</p></div>}
    </section>

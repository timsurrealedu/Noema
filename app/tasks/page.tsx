...
export default function TasksPage(){
  const {tasks,projects,toggleTask,saveTask,archiveTask}=useAppState();
  const [filter,setFilter]=useState<string>("All");
  const [draft,setDraft]=useState<Task|null>(null);

  useEffect(()=>{
    const id=new URLSearchParams(location.search).get("open"),task=tasks.find(item=>item.id===id);
    if(task){setFilter("All");setDraft({...task})}
  },[tasks]);
  ...
  const matches = (task: Task, label: string) => {
    ...
    if (label === "Overdue") return !!task.dueAt && dateValue(task.dueAt) < today;
    if (label === "Today") return dateValue(task.dueAt) === today;
    if (label === "Upcoming") return !!task.dueAt && dateValue(task.dueAt) > today;
  ...
  const ready = tasks.filter(task => !task.completed && !task.archived).length;
  ...
  return (
    <ModuleShell active="Tasks" title="Tasks" action={<button className="primary top-primary" onClick={() => setDraft(blankTask())}><Plus />New task</button>}>
      <div className="module-header">
        <div>
          <h2>What needs doing</h2>
          <p>{ready} task{ready === 1 ? " is" : "s are"} ready.</p>
        </div>
          ...
          <aside className="object-inspector">
            <div className="object-inspector-head">
              <div>
                ...
                <small>{draft.vaultSource ? "Changes rewrite only this Obsidian checklist line." : "Use plain language; changes appear on Today immediately."}</small>
              </div>

...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  let lastEventId=0,closed=false,timer:ReturnType<typeof setInterval>,limit:ReturnType<typeof setTimeout>;
  const stop=(controller:ReadableStreamDefaultController)=>{if(closed)return;closed=true;clearInterval(timer);clearTimeout(limit);try{controller.close()}catch{}};
  const stream=new ReadableStream({
    start(controller){
      const send=(event:string,data:unknown)=>{if(!closed)try{controller.enqueue(encoder.encode(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`))}catch{closed=true}};
      send("state",{id:job.id,kind:job.kind,state:job.state,cancelRequested:!!job.cancel_requested});
      const flush=()=>{
        if(closed)return;
        try{
          for(const row of db.prepare("SELECT * FROM job_events WHERE job_id=? AND id>? ORDER BY id").all(id,lastEventId) as {id:number;type:string;data_json:string;created_at:string}[]){lastEventId=row.id;send(row.type,{id:row.id,type:row.type,data:JSON.parse(row.data_json),createdAt:row.created_at})}
    ...
    cancel(){closed=true;clearInterval(timer);clearTimeout(limit)},
  });

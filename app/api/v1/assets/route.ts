...
export async function POST(request:Request){
  try{
    ...
    const assets=[];
    for(const file of files)assets.push(await storeAsset({stream:file.stream(),name:file.name,mime:file.type||"application/octet-stream"},undefined,undefined,user.workspace.id));

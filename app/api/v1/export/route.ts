...
export async function GET(request:Request){
  try{
    ...
    const date=new Date().toISOString().slice(0,10);return new Response(Readable.toWeb(pack) as ReadableStream,{headers:{"Cache-Control":"no-store","Content-Disposition":`attachment; filename=noema-workspace-${date}.tar`,"Content-Type":"application/x-tar","X-Content-Type-Options":"nosniff"}});

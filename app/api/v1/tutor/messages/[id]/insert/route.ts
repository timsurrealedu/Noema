...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){try{const user=requireWorkspace(request,"editor"),{id}=await params,input=await body(request),actor={id:user.id,workspaceId:user.workspace.id},result=idempotent(request,`${user.id}:${user.workspace.id}`,{id,noteId:input.noteId},()=>insertTutorMessage(id,input.noteId,undefined,actor));return json(result.value)}catch(error){return handle(error)}}

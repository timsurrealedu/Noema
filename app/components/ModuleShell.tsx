...
const nav = [
  ...
  ["Study","/study",BookOpen],["Coding","/coding",Code],["Automations","/automations",Lightning],["Dashboards","/dashboards",SquaresFour],["Plugins","/plugins",Sparkle]
] as const;
...
export function ModuleShell({active,title,action,assistantContext,children}:{active:string;title:string;action?:ReactNode;assistantContext?:{type:string;id:string};children:ReactNode}) {
  ...
  useEffect(()=>{const area=active.toLowerCase(),allowed=new Set(["today","capture","tasks","calendar","vault","graph","study","projects","coding","automations","dashboards","plugins","settings"]);if(allowed.has(area))void fetch("/api/v1/analytics",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:"navigation",properties:{area}})}).catch(()=>{})},[active]);

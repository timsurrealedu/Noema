...
export function ModuleShell({active,title,action,assistantContext,children}:{active:string;title:string;action?:ReactNode;assistantContext?:{type:string;id:string};children:ReactNode}) {
  ...
  return <div className="module-shell">
    ...
    <nav className="mobile-nav" aria-label="Mobile navigation">{([["Today","/",House],["Capture","/capture",Plus],["Tasks","/tasks",ListChecks],["Vault","/vault",Folder],["Coding","/coding",Code],["Automations","/automations",Lightning],["More","/settings",Command]] as const).map(([label,href,Icon],i)=><Link className={`${label===active?"active":""} ${i===1?"capture-nav":""}`} href={href} key={label}><Icon/><span>{label}</span></Link>)}</nav>

...
ssh -i "$KEY" "$HOST" 'export PATH=$HOME/noema/.node-v24.14.0/bin:$PATH && cd ~/noema && rm -rf .next && npm install --no-audit --no-fund --loglevel=error && npm run build'

ssh -i "$KEY" "$HOST" 'pm2 restart noema noema-worker'
sleep 4

status="$(ssh -i "$KEY" "$HOST" "curl -sk -o /dev/null -w '%{http_code}' '$URL/calendar'")"
[ "$status" = "200" ] || { echo "deploy failed: /calendar returned $status"; exit 1; }

leaked="$(ssh -i "$KEY" "$HOST" "grep -rl \"$(id -un)\" ~/noema/.next/server/chunks 2>/dev/null | wc -l")"
[ "$leaked" = "0" ] || { echo "deploy failed: builder paths leaked into bundle"; exit 1; }

probe="$(ssh -i "$KEY" "$HOST" "curl -sk '$URL/api/v1/events/x/occurrences?start=2026-08-01T00:00:00Z&end=2026-08-30T00:00:00Z'")"
case "$probe" in
  "{"*) : ;;

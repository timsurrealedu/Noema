...
test("Vault folder drawer starts closed on tablet and mobile",()=>{const organizer=read("app/components/VaultOrganizer.tsx");assert.match(organizer,/\[drawer,setDrawer\]=useState\(false\)/);assert.match(organizer,/matchMedia\("\(max-width: 900px\)"\)\.matches\?false/)});

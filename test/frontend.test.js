...
test("tablet capture and navigation controls expose a distinct universal action",()=>{const today=read("app/page.tsx"),shell=read("app/components/ModuleShell.tsx"),css=read("app/globals.css");assert.match(today,/name="quick-capture"/);assert.match(today,/autoComplete="off"/);assert.match(today,/\["Coding","\/coding",Code\]/);assert.match(shell,/\["Coding","\/coding",Code\]/);assert.match(css,/\.mobile-nav \.capture-nav svg\{[^}]*width:44px[^}]*border-radius:50%[^}]*background:var\(--primary\)/);assert.match(css,/\.capture-tool\{display:grid/)});
...
test("Home centers attention and capture while secondary task creation stays compact",()=>{
  const home=read("app/page.tsx"),capture=read("app/capture/page.tsx"),calendar=read("app/calendar/page.tsx"),css=read("app/globals.css");
  ...
  assert.match(home,/className="secondary top-primary home-new-task"/);

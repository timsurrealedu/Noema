...
test("Home formats canonical task timestamps instead of displaying ISO strings",()=>{const home=read("app/page.tsx");assert.match(home,/taskDue/);assert.match(home,/<time>\{taskDue\(task\)\}<\/time>/)});
...
test("task view selector is functional and selected view is not repeated on mobile",()=>{const tasks=read("app/page.tsx"),css=read("app/globals.css");assert.match(tasks,/<select[^>]*aria-label="Task view"/);assert.match(tasks,/onChange=\{event=>setFilter\(event\.target\.value\)\}/);assert.match(css,/\.task-list \.list-title h3\{display:none\}/)});
...
test("Vault defers heavy note tools until a note opens",()=>{const vault=read("app/vault/page.tsx");assert.match(vault,/dynamic\(\(\)=>import\("\.\.\/components\/MarkdownContent"\)/);assert.match(vault,/dynamic\(\(\)=>import\("\.\.\/components\/MixedNoteEditor"\)/);assert.match(vault,/dynamic\(\(\)=>import\("\.\.\/components\/TutorPanel"\)/)});
...
test("Vault folder drawer starts closed on tablet and mobile",()=>{const organizer=read("app/components/VaultOrganizer.tsx");assert.match(organizer,/\[drawer,setDrawer\]=useState\(false\)/);assert.match(organizer,/!matchMedia\("\(max-width: 900px\)"\)\.matches&&/)});
...
test("Settings row selects retain a readable width",()=>assert.match(read("app/globals.css"),/\.settings-content \.setting-row select\{width:auto;min-width:200px\}/));
...
test("Calendar Day view shows the full day in its own scroll area",()=>{const page=read("app/calendar/page.tsx"),css=read("app/globals.css");assert.match(page,/\"00:00\"/);assert.match(page,/\"22:00\"/);assert.match(page,/dayPositionFor/);assert.match(css,/\.day-view\{height:min\(70dvh,680px\);min-height:0;overflow:auto/);assert.match(css,/\.day-view>\.times,\.day-view>div:last-child\{min-height:1224px/)});
test("Calendar Week view shows the full day in its own scroll area",()=>{const page=read("app/calendar/page.tsx"),css=read("app/globals.css");assert.match(page,/\{dayTimes\.map/);assert.match(css,/\.week-body\{height:min\(70dvh,680px\);overflow-y:auto;overflow-x:hidden/);assert.match(css,/\.week-body>\.times,\.week-grid\{min-height:1224px/)});
...
test("Week events use their assigned column width",()=>{const page=read("app/calendar/page.tsx"),css=read("app/globals.css");assert.match(page,/const weekItemStyle/);assert.match(page,/left: weekColumnWidths\.slice\(0, day\)/);assert.match(page,/width: weekColumnWidths\[day\] - 8/);assert.doesNotMatch(css,/\.week-grid>\.calendar-event\{left:4px;right:4px/)});
test("Week columns use bounded content widths and can scroll horizontally",()=>{const page=read("app/calendar/page.tsx"),css=read("app/globals.css");assert.match(page,/Math\.min\(320, Math\.max\(140, 72 \+ longestTitle \* 7\)\)/);assert.match(page,/"--week-columns": weekColumns/);assert.match(css,/\.week\{overflow-x:scroll/);assert.match(css,/\.week-body\{height:min\(70dvh,680px\);overflow-y:auto;overflow-x:hidden/);assert.match(css,/grid-template-columns:var\(--week-columns\)/)});

...
test("Home gives mobile task titles the top row and dates a full-width second row",()=>{const home=read("app/page.tsx"),css=read("app/globals.css");assert.match(home,/due\.slice\(0,3\)\}, \$\{due\.slice/);for(const rule of [/grid-template-columns:28px minmax\(0,1fr\) auto/,/\.task-list \.task-copy\{grid-column:2;grid-row:1\}/,/\.task-list article time\{grid-column:2\/4;grid-row:2;justify-self:end;text-align:right;white-space:nowrap\}/])assert.match(css,rule)});
...
test("mobile task due metadata sits beneath its Edit action",()=>{const css=read("app/globals.css");assert.match(css,/\.task-list article time\{grid-column:2\/4;grid-row:2;justify-self:end;text-align:right;white-space:nowrap\}/)});

...
test("shared navigation groups secondary modules in an accessible More menu",()=>{const shell=read("app/components/ModuleShell.tsx"),nav=read("app/components/MobileNavigation.tsx"),css=read("app/globals.css");for(const label of ["Coding","Study","Projects","Automations","Dashboards","Plugins","Collaboration","Help","Settings"]){assert.match(shell,new RegExp(`\\["${label}"`));assert.match(nav,new RegExp(`\\["${label}"`))}assert.match(nav,/ariaLabel="More navigation"/);assert.match(nav,/aria-haspopup="dialog"/);assert.match(nav,/primaryNav\.map/);assert.match(css,/\.more-dialog>nav/)});
test("compact module sidebar hides the nested Settings label",()=>{const css=read("app/globals.css");assert.match(css,/\.sidebar-footer \.settings span\{display:none!important\}/)});
test("Home mobile navigation matches the shared four-item bar and More menu",()=>{const nav=read("app/components/MobileNavigation.tsx");assert.match(nav,/\["Home","\/",House\]/);assert.match(nav,/ariaLabel="More navigation"/);assert.match(nav,/aria-haspopup="dialog"/);assert.doesNotMatch(nav,/\[\["Home","\/",House\][^\n]+\["Coding","\/coding",Code\]\]/)});
test("mobile navigation persists in the root layout across routes",()=>{const layout=read("app/layout.tsx"),home=read("app/page.tsx"),shell=read("app/components/ModuleShell.tsx");assert.match(layout,/<MobileNavigation\/>/);assert.doesNotMatch(home,/<nav className="mobile-nav"/);assert.doesNotMatch(shell,/<nav className="mobile-nav"/)});
test("mobile navigation is isolated from app-state hydration",()=>{const layout=read("app/layout.tsx");assert.match(layout,/<MobileNavigation\/><AppStateProvider>/)});
test("Home desktop navigation uses More for Coding and has no theme shortcut",()=>{const home=read("app/page.tsx"),shell=read("app/components/ModuleShell.tsx"),settings=read("app/settings/page.tsx");assert.match(home,/primaryNav\.slice\(0,4\)/);assert.match(home,/aria-label="More navigation"/);assert.match(home,/\["Coding","\/coding",Code\]/);assert.doesNotMatch(home,/theme-toggle/);assert.doesNotMatch(shell,/theme-toggle/);assert.match(settings,/Theme/)});
test("capture desktop layout gives each pane its own bounded scroll region",()=>{const css=read("app/globals.css");assert.match(css,/\.module-shell:has\(\.capture-inbox\) \.module-main\{[^}]*height:calc\(100dvh - 64px\);[^}]*overflow:hidden/);assert.match(css,/\.module-shell:has\(\.capture-inbox\) \.capture-list-pane\{[^}]*overflow-y:auto/);assert.match(css,/\.module-shell:has\(\.capture-inbox\) \.capture-inspector\{[^}]*position:static;[^}]*height:100%;[^}]*overflow-y:auto/);assert.match(css,/\.capture-queue-toolbar\{[^}]*z-index:20;[^}]*background-color:var\(--bg\)/)});
...
test("root layout applies the saved Gruvbox theme before hydration",()=>{const layout=read("app/layout.tsx");assert.match(layout,/beforeInteractive/);assert.match(layout,/noema-theme/);assert.match(layout,/dataset\.theme/);assert.match(layout,/theme-color/);assert.match(layout,/<html lang="en" data-scroll-behavior="smooth" suppressHydrationWarning>/)});
...
test("mobile navigation exposes core navigation items",()=>{const nav=read("app/components/MobileNavigation.tsx");assert.match(nav,/\["Home"/);assert.match(nav,/\["Vault"/)});
test("tablet capture and navigation controls use a consistent icon system",()=>{const today=read("app/page.tsx"),nav=read("app/components/MobileNavigation.tsx"),css=read("app/globals.css");assert.match(today,/name="quick-capture"/);assert.match(today,/autoComplete="off"/);assert.match(nav,/\["Capture","\/capture",(Plus|Tray)\]/);assert.match(css,/\.mobile-nav a svg\{width:22px;height:22px\}/);assert.doesNotMatch(`${today}${nav}${css}`,/capture-nav/);assert.match(css,/\.capture-tool\{display:grid/)});
...
test("Vault note history returns mobile back gestures to the containing folder",()=>{const vault=read("app/vault/page.tsx");assert.match(vault,/history\.pushState\(null,"",`\$\{location\.pathname\}\?\$\{params\}`\)/);assert.match(vault,/addEventListener\("popstate",restore\)/);assert.match(vault,/setFolder\(params\.get\("folder"\)\|\|""\)/);assert.match(vault,/if\(!open\)\{setDraft\(null\);return\}/)});
...
test("note attachments use canonical assets and insert Markdown at the caret",()=>{const attachment=read("app/components/NoteAttachmentButton.tsx"),vault=read("app/vault/page.tsx");assert.match(attachment,/\/api\/v1\/assets/);assert.match(attachment,/setRangeText/);assert.match(attachment,/image\//);assert.match(read("app/components/LiveMarkdownEditor.tsx"),/onUpload: uploadImage/)});
...
test("notes share explicit modes while tutor and vault panes remain adjustable",()=>{const tutor=read("app/components/TutorPanel.tsx"),mixed=read("app/components/MixedNoteEditor.tsx"),vault=read("app/vault/page.tsx"),organizer=read("app/components/VaultOrganizer.tsx");assert.match(tutor,/MarkdownContent text=\{variant\?\.answer\?\?message\.text\}/);assert.match(tutor,/resizeWidth/);for(const mode of ["minimized","sheet","full"])assert.match(tutor,new RegExp(`"${mode}"`));assert.match(mixed,/viewMode/);assert.match(mixed,/Note mode/);assert.match(organizer,/noema-vault-drawer/);assert.match(organizer,/aria-expanded=\{drawer\}/)});
test("vault backing detection remains independent of the shared editor",()=>{const ts=require("typescript"),compiled=ts.transpileModule(read("app/lib/noteKind.ts"),{compilerOptions:{module:ts.ModuleKind.CommonJS}}).outputText,mod={exports:{}};new Function("exports","module",compiled)(mod.exports,mod);const {isVaultBackedNote}=mod.exports,vault=read("app/vault/page.tsx");assert.equal(isVaultBackedNote({sourceId:"source",relativePath:"folder/note.md"}),true);for(const note of [{id:"local",source:"Created in Noema"},{sourceId:"source",relativePath:null},{sourceId:null,relativePath:"note.md"},null])assert.equal(isVaultBackedNote(note),false);assert.doesNotMatch(vault,/if\(isVaultNote&&draft\)/);assert.match(vault,/<MixedNoteEditor key=\{draft.id\}/)});
...
test("Vault opens Tutor without a deferred component load",()=>{const vault=read("app/vault/page.tsx");assert.match(vault,/dynamic\(\(\)=>import\("\.\.\/components\/MixedNoteEditor"\)/);assert.match(vault,/import \{TutorPanel\} from "\.\.\/components\/TutorPanel"/);assert.doesNotMatch(vault,/dynamic\(\(\)=>import\("\.\.\/components\/TutorPanel"\)/)});
...
test("Calendar Week positions events in their stored timezone",()=>{const page=read("app/calendar/page.tsx"),calendar=read("app/lib/calendar.ts");assert.match(calendar,/timeZone/);assert.match(page,/minutesAt\(event\.startAt,event\.timezone\)/);assert.match(page,/weekPosition/);const compiled=require("typescript").transpileModule(calendar,{compilerOptions:{module:require("typescript").ModuleKind.CommonJS}}).outputText,mod={exports:{}};new Function("exports","module",compiled)(mod.exports,mod);assert.equal(mod.exports.minutesAt("2026-08-26T03:00:00.000Z","Asia/Jakarta"),600);assert.equal(mod.exports.minutesAt("2026-08-26T03:00:00.000Z","UTC"),180)});

test("Applying a capture proposal carries full calendar fields into local state",()=>{
  const state=read("app/components/AppState.tsx");
  // Week view positions blocks from startAt/endAt/allDay/timezone; omitting them collapses the block until refresh.
  assert.match(state,/startAt:object\.startAt\?\?object\.start_at\?\?undefined/);
  assert.match(state,/endAt:object\.endAt\?\?object\.end_at\?\?undefined/);
  assert.match(state,/allDay:object\.allDay\?\?object\.all_day/);
...
test("Week touch gestures allow horizontal and vertical panning",()=>{const css=read("app/globals.css");assert.match(css,/\.calendar-layout>\.week \.week-day-column\{touch-action:auto!important\}/)});
...
test("Calendar collision layout includes events and tasks in Day and Week",()=>{const page=read("app/calendar/page.tsx");assert.match(page,/dayTimedItems=overlapLayout/);assert.match(page,/weekTimedItems=.*overlapLayout/);assert.match(page,/item\.lane\/item\.lanes/)});
test("mobile event editor stacks its header above its form",()=>{const css=read("app/globals.css");assert.match(css,/\.calendar-inspector\{[^}]*display:flex;[^}]*flex-direction:column/)});
...
test("Vault notes support focused fullscreen reading",()=>{const page=read("app/vault/page.tsx"),css=read("app/globals.css");assert.match(page,/onClick=\{toggleFullscreen\}/);assert.match(css,/note-workspace\.fullscreen/);assert.match(css,/mobile-nav\{display:none\}/)});
...
test("Home centers attention and capture while task creation floats",()=>{
  ...
  assert.doesNotMatch(capture,/mobile-action-dock/);assert.match(capture,/Quick capture/);
...
test("the shared formatting toolbar uses native controls and existing editor commands",()=>{
  const live=read("app/components/LiveMarkdownEditor.tsx");
  assert.match(live,/role="toolbar" aria-label="Text formatting"/);
  assert.match(live,/select aria-label="Text style"/);
  for(const command of ["toggleStrongCommand","toggleEmphasisCommand","createTable","undoCommand","redoCommand"])assert.match(live,new RegExp(command));
  assert.match(live,/\[Crepe.Feature.TopBar\]:false/);
});

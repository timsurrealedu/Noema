...
test("Today creates handwriting only after the capture dialog is submitted",()=>{const today=read("app/page.tsx"),capture=read("app/components/HandwritingCapture.tsx"),css=read("app/globals.css");assert.match(today,/aria-label="Write a handwritten note"/);assert.match(today,/<HandwritingCapture/);assert.doesNotMatch(today,/\/vault\?new=ink/);assert.match(today,/capture-tool capture-voice/);assert.match(css,/@media\(max-width:390px\)\{\.app-shell \.capture-voice\{display:none\}\}/);assert.doesNotMatch(css,/capture-handwriting\{display:none/);for(const value of ["Quick note","Choose folder","Draft","/api/v1/handwriting-notes","Done"])assert.match(capture,new RegExp(value));assert.match(capture,/disabled=\{[^}]*!strokes\.length/)});

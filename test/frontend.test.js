...
test("handwriting notes expose a durable authenticated intake endpoint",()=>{const route=read("app/api/v1/handwriting-notes/route.ts"),service=read("server/handwriting.mjs");assert.match(route,/requireWorkspace\(request,"editor"\)/);assert.match(route,/idempotent/);assert.match(service,/queueOcr:state===\"queued\"/);assert.match(service,/state===\"done\"\?\"confirmed\"/)});

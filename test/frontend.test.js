...
test("capture desktop layout keeps the inspector viewport-bound and queue opaque",()=>{const css=read("app/globals.css");assert.match(css,/\.capture-inspector\{[^}]*position:sticky;top:0;[^}]*height:calc\(100dvh - 64px\);[^}]*overflow:auto/);assert.match(css,/\.capture-queue-toolbar\{[^}]*z-index:20;[^}]*background-color:var\(--bg\)/)});

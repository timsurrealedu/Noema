...
test("Home abbreviates mobile task weekdays and keeps source text below the title",()=>{const home=read("app/page.tsx"),css=read("app/globals.css");assert.match(home,/due\.slice\(0,3\)/);assert.match(home,/<time>\{mobileTaskDue\(task\)\}<\/time>/);assert.match(css,/\.task-list \.task-copy\{grid-column:2;grid-row:1\/3;align-self:stretch;justify-content:space-between\}/)});
...
test("mobile task due metadata sits beneath its Edit action",()=>{const css=read("app/globals.css");assert.match(css,/\.task-list article time\{grid-column:3;grid-row:2;justify-self:end;text-align:right\}/)});

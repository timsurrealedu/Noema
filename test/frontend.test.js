...
test("Home keeps mobile task dates on one line and aligns the task row",()=>{const home=read("app/page.tsx"),css=read("app/globals.css");assert.match(home,/due\.slice\(0,3\)\}, \$\{due\.slice/);assert.match(home,/<time>\{mobileTaskDue\(task\)\}<\/time>/);for(const rule of [/grid-template-columns:28px minmax\(0,1fr\) max-content/,/\.task-list \.task-check\{grid-row:1\/3;align-self:start;margin-top:4px\}/,/\.task-list article time\{grid-column:3;grid-row:2;justify-self:end;align-self:end;text-align:right;white-space:nowrap\}/,/padding-block:12px 20px/])assert.match(css,rule)});
...
test("mobile task due metadata sits beneath its Edit action",()=>{const css=read("app/globals.css");assert.match(css,/\.task-list article time\{grid-column:3;grid-row:2;justify-self:end;align-self:end;text-align:right;white-space:nowrap\}/)});

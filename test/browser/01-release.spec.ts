...
test("mobile navigation persists and fixed actions never cover scrollable content",async({page})=>{
  ...
  const capture=await page.evaluate(()=>{const pane=document.querySelector<HTMLElement>(".capture-list-pane")!,dock=document.querySelector<HTMLElement>(".capture-mobile-dock")!,nav=document.querySelector<HTMLElement>(".mobile-nav")!;pane.scrollTop=pane.scrollHeight;return {paddingBottom:parseFloat(getComputedStyle(pane).paddingBottom),overflowX:getComputedStyle(pane).overflowX,dockBottom:dock.getBoundingClientRect().bottom,navTop:nav.getBoundingClientRect().top,scrollBottom:pane.scrollHeight-pane.scrollTop-pane.clientHeight}});expect(capture.paddingBottom).toBeGreaterThanOrEqual(64);expect(capture.overflowX).toBe("hidden");expect(Math.abs(capture.dockBottom-capture.navTop)).toBeLessThanOrEqual(1);expect(capture.scrollBottom).toBeLessThanOrEqual(1);
});

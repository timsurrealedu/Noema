...
test("mobile primary actions respect hierarchy, geometry, and touch targets",async({page})=>{
  ...
  for(const [route,label] of [["/capture","Quick capture"],["/calendar","New event"]] as const){
    await page.goto(route);const dock=page.locator(".mobile-action-dock"),action=dock.getByRole(route==="/capture"?"link":"button",{name:label});
...
test("mobile calendar keeps timeline endpoints and month-end dates reachable",async({page})=>{await page.setViewportSize({width:320,height:700});await login(page);for(const [view,selector] of [["Day",".day-view"],["Week",".week-scroll"]] as const){await page.goto("/calendar");await page.evaluate(v=>localStorage.setItem("noema-calendar-view",v),view);await page.reload();await page.waitForSelector(selector);const result=await page.evaluate(selector=>{const scroller=document.querySelector<HTMLElement>(selector)!,times=[...scroller.querySelectorAll<HTMLElement>(".times time")];scroller.scrollTop=0;const first=times[0].getBoundingClientRect(),area=scroller.getBoundingClientRect();scroller.scrollTop=scroller.scrollHeight;const last=times.at(-1)!.getBoundingClientRect();return {firstTop:first.top,areaTop:area.top,lastBottom:last.bottom,areaBottom:area.bottom,last:times.at(-1)!.textContent}} ,selector);expect(result.firstTop).toBeGreaterThanOrEqual(result.areaTop);expect(result.last).toBe("24:00");expect(result.lastBottom).toBeLessThanOrEqual(result.areaBottom)}await page.evaluate(()=>localStorage.setItem("noema-calendar-view","Month"));await page.goto("/calendar");await page.reload();await page.waitForSelector(".month-view");const month=await page.evaluate(()=>{const target=[...document.querySelectorAll<HTMLElement>(".month-view button")].find(button=>button.querySelector("span")?.textContent==="31")!,scroller=document.querySelector<HTMLElement>(".month-scroll")!;scroller.scrollTop=scroller.scrollHeight;const area=scroller.getBoundingClientRect();return {bottom:target.getBoundingClientRect().bottom,areaBottom:area.bottom,scrollHeight:scroller.scrollHeight,clientHeight:scroller.clientHeight}});expect(month.scrollHeight).toBeGreaterThan(month.clientHeight);expect(month.bottom).toBeLessThanOrEqual(month.areaBottom)});

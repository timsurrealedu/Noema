...
test("mobile primary actions respect hierarchy, geometry, and touch targets",async({page})=>{
  ...
  for(const width of [320,375,390,430]){await page.setViewportSize({width,height:844});await page.goto("/");const home=await page.evaluate(()=>{const input=document.querySelector<HTMLInputElement>(".capture input[type=text]")!,inputBox=input.getBoundingClientRect(),capture=document.querySelector<HTMLElement>(".capture")!.getBoundingClientRect(),fab=document.querySelector<HTMLElement>(".home-task-fab")!.getBoundingClientRect(),nav=document.querySelector<HTMLElement>(".mobile-nav")!.getBoundingClientRect(),captureNav=document.querySelector<HTMLElement>(".mobile-nav .capture-nav")!.getBoundingClientRect(),captureIcon=document.querySelector<SVGElement>(".mobile-nav .capture-nav svg")!,iconBox=captureIcon.getBoundingClientRect(),iconStyle=getComputedStyle(captureIcon),targets=[...document.querySelectorAll<HTMLElement>(".capture-add,.capture-tool,.send,.task-view-select,.home-task-fab,.row-menu")].filter(node=>getComputedStyle(node).display!=="none").map(node=>{const box=node.getBoundingClientRect();return {width:box.width,height:box.height}});return {fontSize:parseFloat(getComputedStyle(input).fontSize),placeholder:input.placeholder,inputWidth:inputBox.width,captureWidth:capture.width,fabWidth:fab.width,fabBottom:fab.bottom,navTop:nav.top,navHeight:nav.height,captureNavWidth:captureNav.width,captureNavHeight:captureNav.height,iconWidth:iconBox.width,iconHeight:iconBox.height,iconRadius:iconStyle.borderRadius,iconBackground:iconStyle.backgroundColor,overflow:document.documentElement.scrollWidth-innerWidth,targets}});expect(home.placeholder).toBe("Capture anything…");expect(home.fontSize).toBeGreaterThanOrEqual(16);expect(home.inputWidth).toBeGreaterThanOrEqual(150);expect(home.captureWidth).toBeLessThanOrEqual(width-32);expect(home.fabWidth).toBeLessThan(180);expect(home.fabBottom).toBeLessThanOrEqual(home.navTop-8);expect(home.navHeight).toBeLessThanOrEqual(72);expect(home.captureNavWidth).toBeGreaterThanOrEqual(44);expect(home.captureNavHeight).toBeGreaterThanOrEqual(44);expect(home.iconWidth).toBe(44);expect(home.iconHeight).toBe(44);expect(home.iconRadius).toBe("50%");expect(home.iconBackground).not.toBe("rgba(0, 0, 0, 0)");expect(home.overflow).toBeLessThanOrEqual(1);for(const target of home.targets){expect(target.width).toBeGreaterThanOrEqual(44);expect(target.height).toBeGreaterThanOrEqual(44)}}

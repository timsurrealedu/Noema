...
test("mobile navigation persists and fixed actions never cover scrollable content",async({page})=>{
  ...
  const capture=await page.evaluate(()=>{const pane=document.querySelector<HTMLElement>(".capture-list-pane")!,nav=document.querySelector<HTMLElement>(".mobile-nav")!;pane.scrollTop=pane.scrollHeight;return {overflowX:getComputedStyle(pane).overflowX,paneBottom:pane.getBoundingClientRect().bottom,navTop:nav.getBoundingClientRect().top,scrollBottom:pane.scrollHeight-pane.scrollTop-pane.clientHeight}});expect(capture.overflowX).toBe("hidden");expect(capture.scrollBottom).toBeLessThanOrEqual(1);
});
...
test("mobile navigation labels and geometry stay stable during Home to Capture navigation",async({page})=>{
  ...
  const sampling=page.evaluate(async()=>{const samples:{height:number;labels:number[];bottom:number;viewportWidth:number;documentWidth:number;scale:number;items:{x:number;y:number;width:number;height:number;transform:string}[]}[]=[];await new Promise<void>(resolve=>{const sample=()=>{const nav=document.querySelector<HTMLElement>(".mobile-nav")!,box=nav.getBoundingClientRect();samples.push({height:box.height,bottom:box.bottom,viewportWidth:visualViewport?.width||innerWidth,documentWidth:document.documentElement.scrollWidth,scale:visualViewport?.scale||1,labels:[...nav.querySelectorAll<HTMLElement>("span")].map(node=>node.getBoundingClientRect().height),items:[...nav.children].map(node=>{const item=node.getBoundingClientRect();return {x:item.x,y:item.y,width:item.width,height:item.height,transform:getComputedStyle(node).transform}})});if(location.pathname==="/capture"&&document.querySelector(".capture-inbox"))resolve();else requestAnimationFrame(sample)};sample()});return samples});
  ...
  expect(samples.length).toBeGreaterThan(1);const first=samples[0];for(const sample of samples){expect(sample.height).toBeGreaterThanOrEqual(68);expect(Math.abs(sample.bottom-844)).toBeLessThanOrEqual(1);expect(sample.documentWidth).toBeLessThanOrEqual(sample.viewportWidth+1);expect(Math.abs(sample.scale-first.scale)).toBeLessThanOrEqual(.01);for(const label of sample.labels)expect(label).toBeGreaterThan(0);sample.items.forEach((item,index)=>{const initial=first.items[index];expect(item.transform).toBe("none");expect(Math.abs(item.x-initial.x)).toBeLessThanOrEqual(1);expect(Math.abs(item.y-initial.y)).toBeLessThanOrEqual(1);expect(Math.abs(item.width-initial.width)).toBeLessThanOrEqual(1);expect(Math.abs(item.height-initial.height)).toBeLessThanOrEqual(1)})}
});

test("mobile calendar editor is one column and concurrent items split into lanes",async({page})=>{
  await page.setViewportSize({width:390,height:844});await login(page);
  await page.evaluate(async()=>{const start=new Date();start.setHours(10,0,0,0);for(const [index,offset] of [0,30].entries()){const from=new Date(start.getTime()+offset*60000),to=new Date(from.getTime()+60*60000),response=await fetch("/api/v1/events",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:`Collision ${index+1}`,startAt:from.toISOString(),endAt:to.toISOString(),timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,allDay:false})});if(!response.ok)throw new Error(await response.text())}});
  await page.goto("/calendar");await page.getByRole("button",{name:"Day",exact:true}).click();
  const first=page.getByRole("button",{name:/Collision 1/}),second=page.getByRole("button",{name:/Collision 2/});await expect(first).toBeVisible();await expect(second).toBeVisible();
  const lanes=await page.evaluate(()=>[...document.querySelectorAll<HTMLElement>('[aria-label="Day schedule"]>button')].filter(node=>node.textContent?.includes("Collision")).map(node=>{const box=node.getBoundingClientRect();return {left:box.left,right:box.right,top:box.top,bottom:box.bottom}}));expect(lanes).toHaveLength(2);expect(Math.min(lanes[0].bottom,lanes[1].bottom)).toBeGreaterThan(Math.max(lanes[0].top,lanes[1].top));expect(lanes[0].right<=lanes[1].left||lanes[1].right<=lanes[0].left).toBe(true);
  await first.click();const editor=await page.evaluate(()=>{const sheet=document.querySelector<HTMLElement>(".calendar-inspector")!,head=sheet.querySelector<HTMLElement>(".object-inspector-head")!,form=sheet.querySelector<HTMLElement>("form")!,box=sheet.getBoundingClientRect(),headBox=head.getBoundingClientRect(),formBox=form.getBoundingClientRect();return {left:box.left,right:box.right,width:box.width,headBottom:headBox.bottom,formTop:formBox.top,formWidth:formBox.width}});expect(editor.left).toBeLessThanOrEqual(1);expect(editor.right).toBeGreaterThanOrEqual(389);expect(editor.headBottom).toBeLessThanOrEqual(editor.formTop);expect(editor.formWidth).toBeGreaterThan(editor.width-40);
});
...
test("mobile Week timeline accepts horizontal scrolling",async({page})=>{await page.setViewportSize({width:390,height:844});await login(page);await page.goto("/calendar");await page.getByRole("button",{name:"Week",exact:true}).click();const scroller=page.locator(".week-scroll");await expect(scroller).toBeVisible();const before=await scroller.evaluate(node=>({width:node.clientWidth,scrollWidth:node.scrollWidth,touch:getComputedStyle(node.querySelector(".week-day-column")!).touchAction}));expect(before.scrollWidth).toBeGreaterThan(before.width);expect(before.touch).toBe("auto");await scroller.evaluate(node=>node.scrollLeft=240);expect(await scroller.evaluate(node=>node.scrollLeft)).toBeGreaterThan(0)});

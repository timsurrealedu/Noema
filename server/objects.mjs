...
export async function storeAsset({stream,name,mime},config=loadConfig(),db=getDatabase(config)){
  ...
  const existing=db.prepare("SELECT * FROM assets WHERE sha256=?").get(sha256);if(existing)return {...existing,kind,deduplicated:true};
...
export function getAsset(id,db=getDatabase()){const asset=db.prepare("SELECT * FROM assets WHERE id=? OR sha256=?").get(id,id);return asset||null}
export function assetsForCapture(captureId,db=getDatabase()){return db.prepare("SELECT a.* FROM capture_assets ca JOIN assets a ON a.id=ca.asset_id WHERE ca.capture_id=?").all(captureId)}
export function attachAssets(captureId,assetIds,db=getDatabase()){
  if(!Array.isArray(assetIds))return;
  for(const assetId of assetIds.slice(0,10)){
    const asset=getAsset(String(assetId),db);if(!asset)throw Object.assign(new Error(`Asset not found: ${assetId}`),{status:404});

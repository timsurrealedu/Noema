...
export function finishJob(id,result,db=getDatabase()){const changed=db.prepare("UPDATE jobs SET state='completed',result_json=?,lease_until=NULL,updated_at=?,version=version+1 WHERE id=? AND state IN ('claimed','running') AND cancel_requested=0").run(JSON.stringify(result),now(),id).changes;if(!changed)throw Object.assign(new Error("Job is not completable"),{status:409});addJobEvent(id,"completed",result,db)}
...
export function cancelJob(id,db=getDatabase()){const job=db.prepare("SELECT state FROM jobs WHERE id=?").get(id);if(!job||!["queued","claimed","running"].includes(job.state))return false;const state=job.state==="queued"?"cancelled":job.state;db.prepare("UPDATE jobs SET cancel_requested=1,state=?,lease_until=CASE WHEN ?='cancelled' THEN NULL ELSE lease_until END,updated_at=?,version=version+1 WHERE id=?").run(state,state,now(),id);addJobEvent(id,state==="cancelled"?"cancelled":"cancel-requested",{},db);return true}
export function assertNotCancelled(id,db=getDatabase()){if(db.prepare("SELECT cancel_requested FROM jobs WHERE id=?").get(id)?.cancel_requested)throw Object.assign(new Error("Job cancelled"),{code:"JOB_CANCELLED"})}
export function retryJob(id,db=getDatabase()){const job=db.prepare("SELECT state,attempts,max_attempts FROM jobs WHERE id=?").get(id);if(!job)throw Object.assign(new Error("Job not found"),{status:404});if(job.state!=="failed")throw Object.assign(new Error("Only failed jobs can be retried"),{status:409,code:"NOT_RETRYABLE"});const time=now();db.prepare("UPDATE jobs SET state='queued',attempts=0,cancel_requested=0,error=NULL,lease_until=NULL,updated_at=?,version=version+1 WHERE id=?").run(time,id);addJobEvent(id,"retry-now",{previousAttempts:job.attempts,maxAttempts:job.max_attempts},db);return getJob(id,db)}

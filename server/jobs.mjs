...
export function retryJob(id,db=getDatabase(),workspaceId=null){const job=db.prepare(`SELECT state,attempts,max_attempts FROM jobs WHERE id=?${workspaceId?" AND workspace_id=?":""}`).get(id,...(workspaceId?[workspaceId]:[]));if(!job)throw Object.assign(new Error("Job not found"),{status:404});if(job.state!=="failed")throw Object.assign(new Error("Only failed jobs can be retried"),{status:409,code:"NOT_RETRYABLE"});const time=now();db.prepare(`UPDATE jobs SET state='queued',attempts=0,cancel_requested=0,error=NULL,lease_until=NULL,updated_at=?,version=version+1 WHERE id=?${workspaceId?" AND workspace_id=?":""}`).run(time,id,...(workspaceId?[workspaceId]:[]));addJobEvent(id,"retry-now",{previousAttempts:job.attempts,maxAttempts:job.max_attempts},db);return getJob(id,db,workspaceId)}
export function getJob(id,db=getDatabase(),workspaceId=null){const job=db.prepare(`SELECT * FROM jobs WHERE id=?${workspaceId?" AND workspace_id=?":""}`).get(id,...(workspaceId?[workspaceId]:[]));if(!job)return null;return {...job,input:JSON.parse(job.input_json),result:job.result_json?JSON.parse(job.result_json):null,events:db.prepare("SELECT * FROM job_events WHERE job_id=? ORDER BY id").all(id).map(e=>({...e,data:JSON.parse(e.data_json)}))}}

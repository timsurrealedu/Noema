...
export async function handleInterpretCapture({job,config,db}){
  try{
    ...
    const prompt=`Interpret this Noema capture into proposed actions. Do not perform actions. Preserve the original language. Use ISO 8601 timestamps with offsets. If required scheduling information is ambiguous, add a clarification and omit that action. Return only the required structured result.\n\nCurrent time: ${now}\nTime zone: ${timezone}\nSource: ${capture.source}\nCapture:\n${capture.text}${attachments.length?`\n\n${attachments.join("\n\n")}`:""}`;
    ...
    assertNotCancelled(job.id,db);const proposal=saveInterpretation(job.input.captureId,output.result,db),objects=proposal.actions.map(action=>{const type=action.type.split(".")[0],args=action.arguments;return {...action,type,title:args.title,detail:type==="event"?`${args.startAt} · ${args.timezone}`:type==="task"?(args.dueAt||args.project||"No due date"):(args.content||"").slice(0,140)}});finishJob(job.id,{provider:output.provider,...proposal,objects},db);return true;

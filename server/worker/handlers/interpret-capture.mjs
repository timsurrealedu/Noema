...
export function isSimpleAutoApply(proposal){const actions=proposal?.actions;return Array.isArray(actions)&&actions.length===1&&!proposal.clarifications?.length&&actions[0].type==="task.create"&&actions[0].confidence>=.9&&!actions[0].arguments?.dueAt&&!actions[0].arguments?.linkedActionId}
...
export async function handleInterpretCapture({job,config,db}){
  try{
    ...
    try{
      ...
      const trustworthy=isSimpleAutoApply(proposal);
  ...
  }catch(error){if(!isTransientAIError(error))db.prepare("UPDATE jobs SET max_attempts=attempts+1 WHERE id=?").run(job.id);failJob(job.id,error,db);const state=db.prepare("SELECT state FROM jobs WHERE id=?").get(job.id)?.state;if(state!=="queued"&&error?.code!=="VERSION_CONFLICT")db.prepare("UPDATE captures SET status=?,error=?,updated_at=? WHERE id=?").run(job.input.revision?"review":"failed",String(error).slice(0,1000),new Date().toISOString(),job.input.captureId)}
}

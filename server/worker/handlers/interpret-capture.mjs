...
export async function handleInterpretCapture({job,config,db}){
  try{
    ...
    assertNotCancelled(job.id,db);const proposal=saveInterpretation(job.input.captureId,validateProposal(output.result,sources),db);finishJob(job.id,{provider:output.provider,...proposal},db);

...
import {retrieveSkillContext} from "../context.mjs";
...
export async function handleRunSkill({job,config,db}){
  ...
  try{
    ...
    const context=retrieveSkillContext({query:job.input.input,skill:job.input.skill,projectId:job.input.projectId,courseId:job.input.courseId,limit:12,db,workspaceId:job.workspace_id});

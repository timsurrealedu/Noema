...
import {workspaceAccess} from "./collaboration.mjs";

export const sessionCookie="lifeos_session";
export const workspaceCookie="lifeos_workspace";
...
export function requireWorkspace(request,minimum="viewer"){const user=requireUser(request),requested=request.headers.get("x-lifeos-workspace")||decodeURIComponent(cookie(request,workspaceCookie)||"");return {...user,workspace:workspaceAccess(user.id,requested,minimum)}}

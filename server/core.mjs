...
export function listState(db=getDatabase(),workspaceId=null){const filter=workspaceId?" WHERE workspace_id=?":"",args=workspaceId?[workspaceId]:[];
  ...
  const tasks=taskRows.map(row=>taskProjectionFromMap(row,vaultTaskLinkMap)),notes=noteRows.map(row=>noteProjectionFromMap(row,vaultSourceMap,noteBlocksMap)),events=db.prepare(`SELECT * FROM events WHERE deleted_at IS NULL${workspaceId?" AND workspace_id=?":""} ORDER BY start_at`).all(...args).map(eventRow);
  return {
    tasks,
    events,
    notes,
    captures:captureRows.map(row=>({id:row.id,text:row.text,source:row.source,status:row.status,sourceLabel:row.source_label,objects:captureObjects(row.objects_json),error:row.error,assets:(captureAssetsMap.get(row.id)||[]).map(asset=>({id:asset.id,name:asset.name,mime:asset.mime,size:asset.size})),createdAt:row.created_at,version:row.version})),

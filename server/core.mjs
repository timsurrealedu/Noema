...
export function listState(db=getDatabase(),workspaceId=null){const filter=workspaceId?" WHERE workspace_id=?":"",args=workspaceId?[workspaceId]:[];
  return {
    tasks:db.prepare(`SELECT * FROM tasks${filter} ORDER BY created_at DESC`).all(...args).map(row=>taskProjection(row,db)),
    ...
    calendarItems:[...db.prepare(`SELECT * FROM events WHERE deleted_at IS NULL${workspaceId?" AND workspace_id=?":""} ORDER BY start_at`).all(...args).map(row=>({kind:"event",event:eventRow(row)})),...db.prepare(`SELECT * FROM tasks WHERE archived=0 AND (due_at IS NOT NULL OR scheduled_start_at IS NOT NULL)${workspaceId?" AND workspace_id=?":""} ORDER BY COALESCE(scheduled_start_at,due_at)`).all(...args).map(row=>({kind:"task",task:taskProjection(row,db)}))],
  };
}

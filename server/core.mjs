...
export function listState(db=getDatabase()){
  return {
    ...
    events:db.prepare("SELECT * FROM events WHERE deleted_at IS NULL ORDER BY start_at").all().map(eventRow),
    notes:db.prepare("SELECT * FROM notes ORDER BY updated_at DESC").all().map(row=>({...row,ai:!!row.ai,draft:!!row.draft,favorite:!!row.favorite,trashed:!!row.trashed,tags:parse(row.tags_json),time:row.updated_at})),
...
export function saveEvent(input,db=getDatabase(),actor=null,options={}){
  const id=input.id||randomUUID(),title=required(input.title,"title",500),now=stamp(),before=db.prepare("SELECT * FROM events WHERE id=?").get(id),timezone=required(input.timezone||before?.timezone||"UTC","timezone",100),legacyDay=Number(input.day),legacyTime=String(input.time||"09:00");
  if(input.googleCalendarId&&actor&&!db.prepare("SELECT 1 FROM google_calendars c JOIN google_accounts a ON a.id=c.account_id WHERE a.user_id=? AND c.calendar_id=? AND c.selected=1").get(actor,input.googleCalendarId))throw new Error("Select a valid Google calendar");
  requireVersion(input,before);
  ...
  db.prepare(`INSERT INTO events(id,title,day,time,top,height,location,active,reminder_at,start_at,end_at,timezone,all_day,recurrence_json,deleted_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,day=excluded.day,time=excluded.time,top=excluded.top,height=excluded.height,location=excluded.location,active=excluded.active,reminder_sent_at=CASE WHEN events.reminder_at IS excluded.reminder_at THEN events.reminder_sent_at ELSE NULL END,reminder_at=excluded.reminder_at,start_at=excluded.start_at,end_at=excluded.end_at,timezone=excluded.timezone,all_day=excluded.all_day,recurrence_json=excluded.recurrence_json,deleted_at=NULL,updated_at=excluded.updated_at,version=events.version+1`).run(id,title,day,timeValue,Number(input.top)||0,height,input.location||null,input.active?1:0,reminderAt,start.toISOString(),end.toISOString(),timezone,input.allDay?1:0,recurrence,null,before?.created_at||now,now);
  const saved=eventRow(db.prepare("SELECT * FROM events WHERE id=?").get(id));audit(db,before?"update":"create","event",id,title,before?{op:"restore",row:before}:{op:"delete"},actor);if(!options.skipCalendarSync)queueCalendarWrite(db,saved,before?"update":"create",actor,input.googleCalendarId);return saved;
}

export function deleteEvent(id,version,db=getDatabase(),actor=null,options={}){const before=db.prepare("SELECT * FROM events WHERE id=? AND deleted_at IS NULL").get(id);if(!before)throw Object.assign(new Error("Event not found"),{status:404});requireVersion({version},before);const time=stamp();db.prepare("UPDATE events SET deleted_at=?,updated_at=?,version=version+1 WHERE id=?").run(time,time,id);const row=db.prepare("SELECT * FROM events WHERE id=?").get(id);audit(db,"delete","event",id,before.title,{op:"restore",row:before},actor);if(!options.skipCalendarSync)queueCalendarWrite(db,row,"delete",actor);return {ok:true,deletedAt:time,version:before.version+1}}
...
export function saveNote(input,db=getDatabase(),actor=null){
  ...
  requireVersion(input,before);
  db.prepare(`INSERT INTO notes(id,title,excerpt,content,tags_json,ai,draft,source,favorite,trashed,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,excerpt=excluded.excerpt,content=excluded.content,tags_json=excluded.tags_json,ai=excluded.ai,draft=excluded.draft,source=excluded.source,favorite=excluded.favorite,trashed=excluded.trashed,updated_at=excluded.updated_at,version=notes.version+1`).run(id,title,excerpt,content,JSON.stringify(input.tags||[]),input.ai?1:0,input.draft?1:0,input.source||null,input.favorite?1:0,input.trashed?1:0,before?.created_at||time,time);
...
export function requestNoteOptimization(noteId,mode="organize",db=getDatabase()){const note=db.prepare("SELECT id,content,draft FROM notes WHERE id=? AND trashed=0").get(noteId);if(!note)throw Object.assign(new Error("Note not found"),{status:404});if(!note.draft)throw new Error("Only Draft notes can be optimized");if(!["light","organize","study","technical","voice"].includes(mode))throw new Error("Unknown optimization mode");const id=randomUUID(),jobId=enqueueJob("note-optimize",{optimizationId:id,noteId,mode},db),time=stamp();db.prepare("INSERT INTO note_optimizations(id,note_id,job_id,mode,state,before_content,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)").run(id,noteId,jobId,mode,"queued",note.content,time,time);return {id,jobId,state:"queued"}}
export const noteOptimizations=(noteId,db=getDatabase())=>db.prepare("SELECT * FROM note_optimizations WHERE note_id=? ORDER BY created_at DESC LIMIT 20").all(noteId);

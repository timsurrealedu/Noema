...
export function finishNoteOptimization(id,result,provider,db=getDatabase()){
  const proposal=db.prepare("SELECT before_content FROM note_optimizations WHERE id=? AND state='queued'").get(id);
  if(!proposal)throw new Error("Optimization proposal is not queued");
  let content=result.content;
  if(result.changes){
    const changes=[...result.changes].sort((a,b)=>b.start-a.start);
    let boundary=proposal.before_content.length;
    content=proposal.before_content;
    for(const change of changes){if(change.operation!=="replace_range"||change.start>change.end||change.end>boundary)throw new Error("Invalid or overlapping note optimization range");content=content.slice(0,change.start)+change.replacement+content.slice(change.end);boundary=change.start}
  }
  const changed=db.prepare("UPDATE note_optimizations SET state='ready',after_content=?,summary=?,provider=?,base_version=?,changes_json=?,updated_at=? WHERE id=? AND state='queued'").run(String(content||""),String(result.summary||""),provider,result.baseVersion||null,JSON.stringify(result.changes||[]),stamp(),id);

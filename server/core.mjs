...
export function saveEvent(input,db=getDatabase(),actor=null){
  ...
  audit(db,before?"update":"create","event",id,title,before?{op:"restore",row:before}:{op:"delete"},actor);return eventRow(db.prepare("SELECT * FROM events WHERE id=?").get(id));
}

export function deleteEvent(id,version,db=getDatabase(),actor=null){const before=db.prepare("SELECT * FROM events WHERE id=? AND deleted_at IS NULL").get(id);if(!before)throw Object.assign(new Error("Event not found"),{status:404});requireVersion({version},before);const time=stamp();db.prepare("UPDATE events SET deleted_at=?,updated_at=?,version=version+1 WHERE id=?").run(time,time,id);audit(db,"delete","event",id,before.title,{op:"restore",row:before},actor);return {ok:true,deletedAt:time,version:before.version+1}}

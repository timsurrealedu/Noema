...
export function updateCapture(id,status,db=getDatabase(),actor=null){
  if(!["processing","review","confirmed","failed","dismissed"].includes(status))throw new Error("Invalid capture status");const before=db.prepare("SELECT * FROM captures WHERE id=?").get(id);if(!before)throw new Error("Capture not found");db.prepare("UPDATE captures SET status=?,updated_at=?,version=version+1 WHERE id=?").run(status,stamp(),id);audit(db,"status","capture",id,status,{status:before.status},actor);return true;
}

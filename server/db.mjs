...
CREATE TABLE IF NOT EXISTS math_continuations(id TEXT PRIMARY KEY,note_id TEXT NOT NULL REFERENCES notes(id) ON DELETE CASCADE,block_id TEXT NOT NULL REFERENCES note_blocks(id) ON DELETE CASCADE,workspace_id TEXT NOT NULL,analysis TEXT NOT NULL DEFAULT '',continuation TEXT NOT NULL,confidence TEXT,assumptions_json TEXT NOT NULL DEFAULT '[]',state TEXT NOT NULL DEFAULT 'proposed',provider TEXT,error TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,resolved_at TEXT);
CREATE TABLE IF NOT EXISTS audio_transcripts(id TEXT PRIMARY KEY,capture_id TEXT NOT NULL REFERENCES captures(id) ON DELETE CASCADE,asset_id TEXT,content TEXT NOT NULL DEFAULT '',segments_json TEXT NOT NULL DEFAULT '[]',state TEXT NOT NULL DEFAULT 'queued',provider TEXT,model TEXT,duration_seconds REAL,error TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL);
...
CREATE INDEX IF NOT EXISTS math_continuations_note ON math_continuations(note_id,created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS audio_transcripts_capture ON audio_transcripts(capture_id);

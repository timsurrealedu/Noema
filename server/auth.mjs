...
const encryptionKey=value=>{if(!value||value.length<32)throw new Error("LIFEOS_ENCRYPTION_KEY is required for persisted MFA");return createHash("sha256").update(value).digest()};
...
export function beginTotpEnrollment(userId,email,key,db=getDatabase()){const secret=encodeBase32(randomBytes(20)),stamp=now();db.prepare("UPDATE users SET totp_pending_enc=?,totp_pending_at=?,updated_at=? WHERE id=?").run(encrypt(secret,key),stamp,stamp,userId);auditSecurity(db,userId,"mfa-enrollment-start","Started authenticator enrollment");return {secret,uri:`otpauth://totp/${encodeURIComponent(`LifeOS:${email}`)}?secret=${secret}&issuer=LifeOS&algorithm=SHA1&digits=6&period=30`}}

...
export function processPendingHandwriting(workspaceId,db=getDatabase()){
  const rows=db.prepare("SELECT id,ink_block_id FROM handwriting_intakes WHERE workspace_id=? AND state='pending' ORDER BY created_at").all(workspaceId);let queued=0,skipped=0;for(const row of rows){const key=`handwriting-ocr:${row.ink_block_id}`,active=db.prepare("SELECT 1 FROM jobs WHERE dedupe_key=? AND state IN ('queued','claimed','running')").get(key);if(active){skipped++;continue}enqueueJob("handwriting-ocr",{blockId:row.ink_block_id},db,workspaceId,{dedupeKey:key,maxAttempts:3});db.prepare("UPDATE handwriting_intakes SET state='queued',updated_at=? WHERE id=? AND state='pending'").run(new Date().toISOString(),row.id);queued++}return {queued,skipped}
}

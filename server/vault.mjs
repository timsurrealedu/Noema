...
function validateComposition(value){const input=value&&typeof value==="object"?value:{},background=["blank","ruled","grid"].includes(input.background)?input.background:"blank",objects=Array.isArray(input.objects)?input.objects:[];if(objects.length>200)throw fail("Too many composition objects");return {formatVersion:1,background,objects:objects.map((object,index)=>{if(!object||object.type!=="text")throw fail("Invalid composition object");const number=(key,fallback,min,max)=>{const value=Number(object[key]);return Math.min(max,Math.max(min,Number.isFinite(value)?value:fallback))};return {id:String(object.id||`text-${index}`).slice(0,100),type:"text",x:number("x",40,0,10000),y:number("y",40,0,10000),width:number("width",320,80,10000),height:number("height",120,44,10000),z:Math.round(number("z",index,-10000,10000)),markdown:String(object.markdown||"").slice(0,100000)}})}}

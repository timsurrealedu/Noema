...
export function validateStrokes(input){const formatVersion=input.formatVersion===2?2:1,coordinateSpace=formatVersion===2?"world":"canvas",width=Number(input.width),height=Number(input.height),strokes=input.strokes;if(input.coordinateSpace&&input.coordinateSpace!==coordinateSpace)throw fail("Invalid ink coordinate space");if(!Number.isFinite(width)||!Number.isFinite(height)||width<1||height<1||width>10000||height>10000)throw fail("Invalid ink dimensions");if(!Array.isArray(strokes)||strokes.length>2000)throw fail("Invalid stroke count");let points=0;for(const stroke of strokes){if(!Array.isArray(stroke.points)||!stroke.points.length||stroke.points.length>10000)throw fail("Invalid stroke points");if(!["pen","highlighter","eraser","rectangle","ellipse","arrow"].includes(stroke.tool))throw fail("Invalid stroke tool");for(const point of stroke.points){points++;for(const key of ["x","y","pressure","time"]){if(!Number.isFinite(point[key]))throw fail("Invalid stroke point")}if(point.x<0||point.x>width||point.y<0||point.y>height||point.pressure<0||point.pressure>1)throw fail("Stroke point out of bounds")}}const json=JSON.stringify(strokes);if(points>100000||Buffer.byteLength(json)>5_000_000)throw fail("Ink payload too large");return {formatVersion,coordinateSpace,width,height,strokes,json}}
...
export function insertMarkdownBlockAfter(noteId,afterBlockId,markdown,actor,db=getDatabase()){

...
export function loadConfig(env=process.env){
  ...
  const config={
    ...
    aiFastChain:setting(env,"AI_FAST_CHAIN")||"",aiBalancedChain:setting(env,"AI_BALANCED_CHAIN")||"",aiQualityChain:setting(env,"AI_QUALITY_CHAIN")||"",aiTimeoutMs:Number(setting(env,"AI_TIMEOUT_MS")||15000),captureMaxInputChars:Number(setting(env,"AI_CAPTURE_MAX_INPUT_CHARS")||24000),captureMaxOutputTokens:Number(setting(env,"AI_CAPTURE_MAX_OUTPUT_TOKENS")||2000),captureMaxVaultFoldersChars:Number(setting(env,"AI_CAPTURE_MAX_VAULT_FOLDERS_CHARS")||4000),workerPollMs:Number(setting(env,"WORKER_POLL_MS")||1000),
    deepseekApiKey:env.DEEPSEEK_API_KEY||"",deepseekBaseUrl:setting(env,"DEEPSEEK_BASE_URL")||"https://api.deepseek.com",glmApiKey:env.GLM_API_KEY||"",glmBaseUrl:setting(env,"GLM_BASE_URL")||"https://open.bigmodel.cn/api/paas/v4",kimiApiKey:env.KIMI_API_KEY||"",kimiBaseUrl:setting(env,"KIMI_BASE_URL")||"https://api.moonshot.cn/v1",qwenApiKey:env.QWEN_API_KEY||"",qwenBaseUrl:setting(env,"QWEN_BASE_URL")||"https://dashscope-intl.aliyuncs.com/compatible-mode/v1",openrouterApiKey:env.OPENROUTER_API_KEY||"",openrouterBaseUrl:setting(env,"OPENROUTER_BASE_URL")||"https://openrouter.ai/api/v1",openrouterModel:setting(env,"OPENROUTER_MODEL")||"openrouter/free",groqApiKey:env.GROQ_API_KEY||"",groqBaseUrl:setting(env,"GROQ_BASE_URL")||"https://api.groq.com/openai/v1",groqModel:setting(env,"GROQ_MODEL")||"llama-3.1-8b-instant",
  ...
  for(const [name,value] of [["NOEMA_AI_TIMEOUT_MS",config.aiTimeoutMs],["NOEMA_AI_CAPTURE_MAX_INPUT_CHARS",config.captureMaxInputChars],["NOEMA_AI_CAPTURE_MAX_OUTPUT_TOKENS",config.captureMaxOutputTokens],["NOEMA_AI_CAPTURE_MAX_VAULT_FOLDERS_CHARS",config.captureMaxVaultFoldersChars],["NOEMA_WORKER_POLL_MS",config.workerPollMs]])if(!Number.isInteger(value)||value<1)throw new Error(`${name} must be a positive integer`);
  cachedConfig=config;
  return config;
}

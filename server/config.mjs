...
export function loadConfig(env=process.env){
  ...
  const config={
    ...
    codeDir:absolute(setting(env,"CODE_DIR")||"."),repositoryRoots:(setting(env,"REPOSITORY_ROOTS")||setting(env,"CODE_DIR")||".").split(delimiter).filter(Boolean).map(absolute),pluginCatalogs:(setting(env,"PLUGIN_CATALOGS")||"").split(delimiter).filter(Boolean).map(absolute),compilerEnabled:setting(env,"COMPILER_ENABLED")==="true",codexEnabled:setting(env,"CODEX_ENABLED")==="true",

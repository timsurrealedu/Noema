...
export function loadConfig(env=process.env){
  ...
  const config={
    dataDir,dbPath:resolve(dataDir,"lifeos.sqlite"),objectsDir:resolve(dataDir,"objects"),jobsDir:resolve(dataDir,"jobs"),backupsDir:resolve(dataDir,"backups"),pluginsDir:resolve(dataDir,"plugins"),
    ...
    codeDir:absolute(env.LIFEOS_CODE_DIR||"."),repositoryRoots:(env.LIFEOS_REPOSITORY_ROOTS||env.LIFEOS_CODE_DIR||".").split(delimiter).filter(Boolean).map(absolute),pluginCatalogs:(env.LIFEOS_PLUGIN_CATALOGS||"").split(delimiter).filter(Boolean).map(absolute),compilerEnabled:env.LIFEOS_COMPILER_ENABLED==="true",codexEnabled:env.LIFEOS_CODEX_ENABLED==="true",
...
export function ensureDataDirs(config=loadConfig()){
  for(const dir of [config.dataDir,config.objectsDir,config.jobsDir,config.backupsDir,config.pluginsDir])mkdirSync(dir,{recursive:true,mode:0o700});
  return config;
}
